...
app.post('/recipes', jsonParser, (req,res) => {
  ...
  const [name, ingredients] = ['name', 'ingredients'];

  if (!name in req.body || !ingredients in req.body) {
    const message = 'No name or ingredients are found';
    console.log(message);
    return res.status(400).send(message);
  } else {
    const reItem = Recipes.create(req.body.name, req.body.ingredients);
    res.status(201).json(reItem);
  }
});
